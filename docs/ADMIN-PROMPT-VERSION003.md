# SpotLine Admin 개발 가이드 VERSION003
...
- **체험 시스템**: 업주 소개용 ("이런 서비스입니다") - 읽기 전용
- **운영 시스템**: 실제 서비스 ("실제로 사용하세요") - Admin 관리 대상

### 2. Admin 관리 범위 명확화

- **체험 데이터**: 건드리지 말 것 (DemoStore 스키마)
...
interface DashboardData {
  ...
  monthlyExperiences: number; // 이번 달 SpotLine 체험 수
  monthlyScans: number; // 이번 달 실제 QR 스캔 수
  systemStatus: {
    experienceSystem: boolean; // 체험 시스템 상태 (읽기 전용)
    operationalSystem: boolean; // 운영 시스템 상태
    spotlineExperience: boolean; // SpotLine 체험하기 활성화 상태
  };
}
```
...
const ExperienceSettings: React.FC = () => {
  ...
  const handleSaveConfig = async (configData: Partial<ExperienceConfig>) => {
    try {
      ...
      if (response.ok) {
        alert("SpotLine 체험하기 설정이 저장되었습니다.");
        fetchConfig();
      }
    } catch (error) {
  ...
  return (
    <div className="experience-settings">
      <h2>SpotLine 체험하기 설정</h2>

      {availableStores.length === 0 ? (
        <div className="warning">
          <p>⚠️ 운영 매장을 먼저 등록해주세요.</p>
          <p>SpotLine 체험하기 기능을 사용하려면 최소 1개 이상의 운영 매장이 필요합니다.</p>
        </div>
      ) : (
        <ExperienceConfigForm config={config} availableStores={availableStores} onSave={handleSaveConfig} />
      )}
    </div>
  );
};
...
const ExperienceStoreList: React.FC = () => {
  const [experienceStores, setExperienceStores] = useState<ExperienceStore[]>([]);

  return (
    <div className="experience-store-list">
      <div className="warning-banner">
        <h3>⚠️ 주의사항</h3>
        <p>이 데이터들은 업주 소개용 체험 데이터입니다.</p>
        <p>수정하거나 삭제하지 마세요.</p>
      </div>

      <div className="store-grid">
        {experienceStores.map((store) => (
          <div key={store.id} className="experience-store-card readonly">
            <h4>{store.name}</h4>
            <p>QR ID: {store.qrCodeId}</p>
            <p>지역: {store.area}</p>
            <span className="demo-badge">체험 전용</span>
          </div>
        ))}
      </div>
      ...
      <div className="demo-link">
        ...
        <button onClick={() => window.open("/api/demo/experience", "_blank")}>체험해보기</button>
      </div>
    </div>
  );
};
...
interface ExperienceAnalytics {
  totalExperiences: number; // 총 체험 수
  ...
  conversionRate: number; // 체험 → 실제 스캔 전환율
}
```
...
1. **체험 데이터 수정**: DemoStore 스키마의 데이터는 절대 수정하지 말 것
2. **QR 코드 ID 중복**: demo*\* 와 real*\* 접두사 구분 유지
3. **시스템 혼동**: 체험용과 운영용 API를 섞어서 사용하지 말 것
