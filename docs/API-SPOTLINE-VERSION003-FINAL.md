...
SpotLine 백엔드 API 문서입니다. VERSION003-FINAL에서는 시스템 구조를 명확히 구분하여 데모 시스템과 운영 시스템을 분리했습니다.

## 변경사항 (VERSION003-FINAL)

- **시스템 구조 명확화**: 데모 시스템 vs 운영 시스템 완전 분리
- **버튼 네이밍 최종화**: "데모보기" vs "SpotLine 시작"
...
**설명**: 사용자가 SpotLine 서비스를 시작할 때 사용
...
- **제한**: 데모 시스템(DemoStore 스키마)은 읽기 전용
...
1. **시스템 구분**: 데모용(`/api/demo/*`)과 운영용(`/api/stores/*`, `/api/experience`) API를 명확히 구분
2. **통계 수집**: 데모 시스템은 통계 수집하지 않음, 운영 시스템만 Analytics 데이터 수집
3. **QR 코드 ID**: `demo_*` (데모용) vs `real_*` (운영용) 접두사로 구분
4. **Admin 권한**: 운영 시스템만 Admin에서 관리 가능, 데모 시스템은 읽기 전용

### ✅ 올바른 사용법

1. **업주 소개**: `/api/demo/experience` 사용 → "데모보기" 버튼
2. **사용자 시작**: `/api/experience` 사용 → "SpotLine 시작" 버튼
...
```
사용자 시작 → /api/experience → Store 선택 → Analytics 수집
```
...
- **데모 시스템**: "이런 서비스입니다" (업주 소개용) → "데모보기" 버튼
- **운영 시스템**: "실제로 사용하세요" (정식 서비스) → "SpotLine 시작" 버튼
