...
const selectStoreFromConfig = async (config: any): Promise<string> => {
  ...
  try {
    switch (type) {
      case "fixed":
        return settings.fixedStoreQrId || "demo_cafe_001";

      case "random":
        const randomStores = settings.randomStoreQrIds || [];
        if (randomStores.length === 0) return "demo_cafe_001";
...
const selectFromAreaBased = async (settings: any): Promise<string> => {
  ...
  return firstArea.storeQrIds[0] || "demo_cafe_001";
};
...
const selectFromWeighted = async (settings: any): Promise<string> => {
  ...
  // 폴백
  return enabledStores[0].qrId || "demo_cafe_001";
};
...
const getDefaultExperienceResult = async (): Promise<ExperienceResult> => {
  // 새로 생성된 "카페 스팟라인" 매장의 QR 코드 ID 사용
  const defaultQrId = "demo_cafe_001";
...
export const getAvailableStoreQrIds = async (): Promise<string[]> => {
  try {
    ...
    return ["demo_cafe_001", "demo_gallery_001", "demo_dessert_001", "demo_bookcafe_001"]; // 기본값
  }
};
...
export const getStoreQrIdsByArea = async (): Promise<Record<string, string[]>> => {
  try {
    ...
    return {
      gangnam: ["demo_cafe_001", "demo_gallery_001", "demo_dessert_001"],
      hongdae: ["demo_bookcafe_001"],
    };
  }
};
