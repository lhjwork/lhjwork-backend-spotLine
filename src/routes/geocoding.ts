...
// 네이버 지오코딩 API
router.get("/naver", async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get("/unified", async (req: Request, res: Response): Promise<void> => {
  const { address } = req.query;
  ...
  const normalizedAddress = (address as string)
    .trim()
    .replace(/[^\w\s가-힣]/g, " ")
    .replace(/\s+/g, " ");
